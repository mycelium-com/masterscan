...
GridSampler.checkAndNudgePoints=function( image,  points)
		{
			var width = window.qrcode.width;
			var height = window.qrcode.height;
...
GridSampler.sampleGrid3=function( image,  dimension,  transform)
		{
			...
			for (var y = 0; y < dimension; y++)
			{
				...
				try
				{
					for (var x = 0; x < max; x += 2)
					{
						var xpoint = (Math.floor( points[x]) * 4) + (Math.floor( points[x + 1]) * window.qrcode.width * 4);
                        var bit = image[Math.floor( points[x])+ window.qrcode.width* Math.floor( points[x + 1])];
						window.qrcode.imagedata.data[xpoint] = bit?255:0;
						window.qrcode.imagedata.data[xpoint+1] = bit?255:0;
						window.qrcode.imagedata.data[xpoint+2] = 0;
						window.qrcode.imagedata.data[xpoint+3] = 255;
						//bits[x >> 1][ y]=bit;
						if(bit)
...
function ReedSolomonDecoder(field)
{
	...
	this.decode=function(received,  twoS)
	{
			...
			for (var i = 0; i < twoS; i++)
			{
				...
				var xeval = poly.evaluateAt(this.field.exp(dataMatrix?i + 1:i));
				syndromeCoefficients[syndromeCoefficients.length - 1 - i] = xeval;
				if (xeval != 0)
				{
					noError = false;
				}
